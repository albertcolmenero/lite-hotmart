...
export function SeriesFormFields({
  ...
  return (
    <>
      ...
      <Field label="Cover URL" hint="Otherwise derived from first class">
        <input
          name="coverUrl"
          type="url"
        ...
        >
          {orderedClasses.length === 0 ? (
            <p className="p-4 text-sm" style={{ color: "var(--lichen)" }}>
              No classes yet — create some first.
            </p>
          ) : (
      ...
      <Field label="Categories" hint="Group this series with related content">
        {categories.length === 0 ? (
